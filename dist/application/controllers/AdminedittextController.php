       ...
       if($_SERVER["REQUEST_METHOD"]=='POST'){
        ...
        //инициализация пришедших переменных
            ...
            if(empty($error)){
               ...
               $res = $AdminedittextModel->editContact ($phone1,$phone2,$address,$email,$mode,$footer,$maps);
               if($res) {
                   $_SESSION["contact"] = TextModel::getStaticContact();
                   header("Location: /txt/contact/?ok=".urlencode("Контактная информация обновлена!"));
                }else{
                   header("Location: /txt/contact/?er=".urlencode("Ошибка обновления"));
                }
            }else{
                $str = implode("<br />", $error);
                header("Location: /txt/contact/?er=".urlencode($str)); 
            }
